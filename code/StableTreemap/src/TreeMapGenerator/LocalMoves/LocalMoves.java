...
public class LocalMoves implements TreeMapGenerator {
    ...
    @Override
    public TreeMap generateTreeMap(DataMap newDm, Rectangle inputRectangle, int time) {

//        System.out.println("generatedATreemap");
        if (currentDataMap == null) {
            currentTreeMap = generateNewTreeMap(newDm, inputRectangle, time);
        } else {
            ...
            if (!currentDataMap.hasEqualStructure(newDm)) {
                ...
                updateAdditionTreeMap(currentDataMap, newDm, time);
            }
     ...
     * @return
     */
    private TreeMap generateNewTreeMap(DataMap dataMap, Rectangle treeMapRectangle, int time) {
        ApproximationTreeMap approximationTreeMap = new ApproximationTreeMap();
        return approximationTreeMap.generateTreeMap(dataMap, treeMapRectangle, time);
    }
    
//    private TreeMap generateNewTreeMap(DataMap dataMap, Rectangle treeMapRectangle, DataFacilitator dataSet) {
//        Packing PackingTreeMap = new Packing();
//        return PackingTreeMap.generateTreeMap(dataMap, treeMapRectangle, dataSet);
//    }
     ...
     * @param newDm
     */
    private void updateAdditionTreeMap(DataMap currentDm, DataMap newDm, int time) {
        ...
        if (!currentDm.hasChildren() || itemsToBeAdded.size() > currentItems.size()) {
            ...
            TreeMap newParentTm = generateNewTreeMap(newDm, parentTm.getRectangle(), time);
        ...
        for (DataMap addDm : itemsToBeAdded) {
            ...
            if (addDm.hasChildren()) {
                ...
                TreeMap addTm = generateNewTreeMap(addDm, newParentTm.getChildWithLabel(addDm.getLabel()).getRectangle(), time);
                //replace addTm
        ...
        for (DataMap oldDm : currentItems) {
            ...
            if (newChildDm.hasChildren()) {
                ...
                updateAdditionTreeMap(oldDm, newChildDm, time);
            }
...
//        }
    ...
    public String getSimpleName() {
        ...
        return "Local" + maxMoveAmount + "Moves" + repeatAmount + "Repeats";
    }
}
