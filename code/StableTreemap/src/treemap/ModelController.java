...
public class ModelController {
    ...
    public void setTimeoutTreeMap(int time, Rectangle treeMapRectangle, String commandIdentifier) {
        ...
        //get the last
        String key = time - 1 + ";" + dataMap + ";" + treeMapGenerator.getSimpleName();
        currentTreeMap = treeMapStorage.get(key);
        //update to next
        key = time + ";" + dataMap + ";" + treeMapGenerator.getSimpleName();
     ...
     * @return
     */
    public TreeMap getTreeMap(int time, boolean useStored, Rectangle treeMapRectangle, String commandIdentifier, DataMap rootNode) {
        if (dataFacilitator == null || treeMapGenerator == null) {
            return null;
        }
        //Store the initial seed
        ...
        String key = time + ";" + dataMap + ";" + treeMapGenerator.getSimpleName();
        if (useStored) {
            ...
            if (treeMapStorage.containsKey(key)) {
                ...
                currentTreeMap = treeMapGenerator.generateTreeMap(rootNode, treeMapRectangle, time);
                treeMapStorage.put(key, currentTreeMap);
            }
     ...
     * @return
     */
    public TreeMap getNewGeneratedTreeMap(int time, boolean useStored, Rectangle treeMapRectangle, String commandIdentifier, DataMap rootNode) {
        int initialSeed = this.initialMoveStart;
        getTreeMap(time, useStored, treeMapRectangle, commandIdentifier, rootNode);
        this.initialMoveStart = initialSeed;
        return currentTreeMap;
    }

    public TreeMap getMovesTreeMap(int currentTime, boolean useStored, int moveAmount, Rectangle treeMapRectangle, String commandIdentifier,  DataMap rootNode) {
        ...
        if (moveAmount == 0) {
            ...
            getNewGeneratedTreeMap(initialMoveStart, useStored, treeMapRectangle, commandIdentifier,  rootNode);
