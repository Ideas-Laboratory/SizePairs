...
public class Visualiser extends CommandController {
    ...
    protected boolean getTreeMap(int time, boolean useStored, String commandIdentifier, DataMap rootNode) {
        TreeMap nextTreeMap = modelController.getTreeMap(time, useStored, treeMapRectangle, commandIdentifier, rootNode);
        if (nextTreeMap == null) {
            return false;
        } else {
    ...
    public TreeMap getTreeMapWithoutMoves(int currentTime, boolean useStored, DataMap rootNode) {
        return modelController.getMovesTreeMap(currentTime, useStored, 0, treeMapRectangle, "", rootNode);

    }

    public TreeMap getTreeMapWithMoves(int currentTime, int moveAmount, boolean useStored, DataMap rootNode) {
        return modelController.getMovesTreeMap(currentTime, useStored, moveAmount, treeMapRectangle, "", rootNode);
    }

    public TreeMap getNewGeneratedTreeMap(int currentTime, DataMap rootNode) {
        return modelController.getNewGeneratedTreeMap(currentTime, false, treeMapRectangle, "", rootNode);
    }
    ...
    public void recolor(HashMap<String, Color> colors) {
        modelController.getDataFacilitator().recolor(colors);
    }
