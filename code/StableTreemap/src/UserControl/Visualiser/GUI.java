...
public final class GUI extends javax.swing.JFrame {
    ...
    private DataMap rootNode = new DataMap("virtualRoot", 0, null, null);;
    /**
    ...
    public GUI(Visualiser visualiser) {
        ...
        String[] facilitators = new String[]{"m-coffee", "m-names", "Random"};
        dataFacilitatorSelector.setModel(new DefaultComboBoxModel(facilitators));
        dataFacilitatorSelector.setSelectedIndex(1);
        setDataFacilitator("m-names");

        String[] generators = new String[]{"SliceAndDice", "NumberBalancedPartition", "SizeBalancedPartition", "SequenceBalancedPartition", "PivotByMiddle", "PivotBySize", "PivotBySplitSize", "Strip", "StripLookAhead", "Split", "Squarified", "SquarifiedLookAhead", "Spiral", "SpiralLookAhead", "Moore", "Hilbert", "Approximation", "Incremental", "NoMovesIncremental", "Packing"};
        treeMapSelector.setModel(new DefaultComboBoxModel(generators));
        treeMapSelector.setSelectedIndex(8);
    ...
    public void setDataFacilitator(String identifier) {
        ...
        switch (identifier) {
            case "Random":
                ...
                break;
            case "m-names":
                df = new DataFileManagerFast(new File("").getAbsolutePath() + sep + "Data" + sep + "WorldbankHIV", false);
                break;
            case "m-coffee":
                df = new DataFileManagerFast(new File("").getAbsolutePath() + sep + "." + sep + "Data" + sep + "Datasets" + sep + "UnContradeCoffee", false);
                break;
        }
        visualiser.setDataFacilitator(df);
        rootNode = getRootNode(df, rootNode);
        rootNode = decRootNode(df, rootNode);
    }
    ...
    private void initComponents() {
        ...
        setColorButton.setText("setColors");
        ...
        layout.setVerticalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addComponent(Title)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(visualisationPanel, javax.swing.GroupLayout.DEFAULT_SIZE, 722, Short.MAX_VALUE)
                .addGap(30, 30, 30))
    ...
    private void startSimulation() {
        ...
        //if (!treeMapVisualisation.isShowingTreeMap()) {
        visualiser.getTreeMap(currentTime, useStored, "", rootNode);
//        }
...
//        }
    ...
    private void buttonShowNewGeneratedActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonShowNewGeneratedActionPerformed
        int currentTime = (int) timeSpinner.getValue();
        TreeMap tm = visualiser.getNewGeneratedTreeMap(currentTime, rootNode);
    ...
    private void baseLineButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_baseLineButtonActionPerformed
        if (visualiser.getCurrentTreeMap() != null) {
            ...
            RelativeQuadrantStability rqStab = new RelativeQuadrantStability();
            rqStab.getInstability(currentTreemap, baseLine);

            TreeMap newTm = visualiser.getNewGeneratedTreeMap(currentTime + 1, rootNode);
            double ssv = rqStab.getInstability(currentTreemap, newTm);
            double basessvScore = rqStab.getBaselineInstability(currentTreemap, newTm, baseLine);
            System.out.println("baseSSV = " + basessvScore + ";" + "SSV = " + ssv);

            CornerTravelDistance ctd = new CornerTravelDistance();

            double ctdScore = ctd.getInstability(currentTreemap, newTm);
            double basectdScore = ctd.getBaselineInstability(currentTreemap, newTm, baseLine);
            System.out.println("basectd = " + basectdScore + ";" + "ctd = " + ctdScore);
    ...
    private void setColorButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_setColorButtonActionPerformed
        ...
        TreeMap currentTm = visualiser.getCurrentTreeMap();

        Rectangle tmR = visualiser.getTreeMapRectangle();

        HashMap<String, Color> colors = new HashMap();
        for (TreeMap tm : currentTm.getAllChildren()) {
            colors.put(tm.getLabel(), getGradientColor(tm, tmR));
        }

        visualiser.recolor(colors);

    }//GEN-LAST:event_setColorButtonActionPerformed

    private Color getGradientColor(TreeMap tm, Rectangle rect) {
        ...
        double xPercentage = tm.getRectangle().getX() / rect.getWidth();
        double yPercentage = tm.getRectangle().getY() / rect.getHeight();

        double rHor1 = (r2 - r1) * xPercentage + r1;
        double gHor1 = (g2 - g1) * xPercentage + g1;
        double bHor1 = (b2 - b1) * xPercentage + b1;

        double rHor2 = (r4 - r3) * xPercentage + r3;
        double gHor2 = (g4 - g3) * xPercentage + g3;
        double bHor2 = (b4 - b3) * xPercentage + b3;

        double r = (rHor2 - rHor1) * yPercentage + rHor1;
        double g = (gHor2 - gHor1) * yPercentage + gHor1;
        double b = (bHor2 - bHor1) * yPercentage + bHor1;
    ...
    private void showLoadedData() {
        ...
        int time = (int) timeSpinner.getValue();
        String sep = System.getProperty("file.separator");
        TreeMap tm = tmr.readTreeMap(new File(".." + sep + "Data" + sep + "outputTreemap" + sep + "4MovesIncrementalLayout" + sep + "Hystrix.data" + sep + "t" + time + ".rect"));
    ...
    private void getNewTreeMap() {
        ...
        int time = (int) timeSpinner.getValue();
        visualiser.getTreeMap(time, useStored, "Test", rootNode);
    }
